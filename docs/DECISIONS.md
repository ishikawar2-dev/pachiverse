...
- **名称**: 「オーナーチケット NFT」を維持（オンチェーン発行するため）
  - **追記（2026-09-22、オーナー決定）**: 新しく発行するカードの名称は **「OT カード」（Pachiverse OT Card、シンボル PVOT）**。「オーナーチケット会員カード」「Owner Card」は使わない（OT との関係が分かりにくい）。「Owner's Pass」は旧 ERC721 と同名になるため不可。外部ウォレットへの受取（release）は当面案内しない。移管の注意事項に対価禁止の文は入れない（運営の関与事項ではない）。members `ops/OT_MIGRATION_RUNBOOK.md` D-23 / D-24
...
| **D-10 再改訂（2026-09-21）** | **オーナー決定: 返金は不要。22 人はそのまま継続会員として放置する。** (1)〜(3) は行わない（差し引きなし・削除なし）。取込の除外リスト（members D32、2026-09-21 本番配置）にも登録しない。2026-09-21 時点の本番: 22 人全員 `active`、`uni_member=1` が 14 人、OT 保有 9 人・176,013 枚（報告書 §1.2 と一致）。OT カードの初回対象は現在の保有で判定する。D-19（`suspended` は対象外）はこの 22 人には該当しない（全員 active）。返金対象だった申込は旧サイト側の記録のまま（本サイトの保有には反映済みの分も含めて変更しない） |
...
## Decision: OT カード移行の運用判断 D-16〜D-19（2026-09-17）

### Context
members 3d-1（PR #116）の移行 runbook `ops/OT_MIGRATION_RUNBOOK.md` §0.4 が求める判断。材料は contracts の anvil 予行（`ops/ot-rehearsal-result-2026-09-16.md`: 100 件 = 5.0M gas、200 件 = 9.95M gas、約 50k gas/枚、3,931 枚の総 gas ≈ 196M で 1 TX の件数に依存しない）と、鍵管理 runbook §3.14（OT 鍵の KMS 内生成手順）。
...
## Decision: UNI 現代表ヒアリングの確定事項（2026-09-24）

### Context
03 §3「9/15〜9/30 枠組み確認」の UNI 現代表ヒアリングを 2026-09-24 に実施（出席: 高橋代表・石川。質問と回答の原本は `~/Downloads/ヒアリング項目_会計士税理士_UNI現代表_2026-09-15.txt` (2) 節）。同日、株主総会も実施済み。

### Decision
| # | 項目 | 決定 |
|---|---|---|
| U-1 | 引き渡し範囲 | 会員サイト・コントラクト・署名サーバ・NFT アート・公開サイト・メタバースの 6 つ。メタバースを含めることに異論なし |
| U-2 | 価格構造 | 価格は **IP 部分のみ**（保守は含めない）。譲渡代金は UNI から石川への役員貸付金（石川が代表取締役だった当時の処理）と**相殺**し、代金が貸付残高を超える分は 12 ヶ月以内に支払い |
| U-3 | 保守 | 譲渡翌月から 12 ヶ月間は**無償**のサービス提供。以後は自動更新で**月 5 万円**。追加開発は別途相談 |
| U-4 | UNI が重視する資料 | 第三者による評価書 |
| U-5 | 契約形式 | 著作権等の知財譲渡契約。UNI 側の顧問弁護士・税理士による事前確認はしない |
| U-6 | UNI 側の決裁 | 代表決裁（取締役会決議は不要）。取得理由書は石川が起案し代表へ提出。ソフトウェア 5 年償却は代表了承（顧問税理士の見解は未取得） |
| U-7 | 会計データ | 2025/11〜の入金明細を評価人へ提供することに同意 |
| U-8 | 見届け役・緊急時の意思決定者 | 高橋代表。代表に連絡がつかない場合は石川が判断 |
| U-9 | Safe 2-of-3 の鍵 | Ledger 2 台は**いずれも UNI 側が管理**（署名者: 会社 MetaMask＋UNI 管理の Ledger×2。紙の冷蔵鍵は置かない）。DEFAULT_ADMIN 操作は毎回 UNI の Ledger 署名が要り、緊急時に代表へ連絡がつかない間は管理者操作ができない。このリスクは許容 |
| U-10 | 会員データ | DB は UNI/VB の顧客データのまま、譲渡対象はシステムと IP のみ。会員への通知は不要（UNI 現代表の判断。10 §5 の弁護士確認は別途残る） |
...
### Consequences
- 03 §3 の「2027 年 1〜2 月」の作業（工数表 v1.2 確定・評価書・価格提示）は 12 月に前倒し。評価書は監査レポートを待たずに依頼する（03 §2.4 の「監査後」を改めた）
- 09 §2 の 3＋9 ヶ月の期間構成は維持するが、費用は U-3 のとおり無償（09 §5）。09 §2 が前提にする「UNI 側運用者」は置かず運用担当は石川 1 名のまま継続するため、集中引継ぎの内容は見直しが必要（未反映）
- 鍵管理 Part B: 署名者構成を「会社 MetaMask＋オーナー Ledger＋紙の冷蔵鍵」から U-9 に変更。members `ops/SEALED_BACKUP_RUNBOOK.md` の Safe 行、12 §1 の T+3・保守満了行、Part B の手順は構成確定後に書き直す（未反映）。UNI 側での Ledger の PIN・復旧フレーズの保管方法は未決
- 会計士・税理士ヒアリングに 3 問追加（相殺時の所得認識と裏付け書類・無償保守の課税上の扱い・年内譲渡での評価基準日）。txt (1) E-1〜E-3
- 09 §10・12 §7・10 §8・07 §8 #14 の該当行を更新

### Status
Active

## Decision: IP 帰属の前提事実 3 点を確定し、譲渡資料の表現を「企画」に統一する（2026-09-24）

### Context
UNI 側の取得理由書（[13_UNI_ACQUISITION_RATIONALE.md](13_UNI_ACQUISITION_RATIONALE.md)）の起案で、IP 帰属の説明に必要な事実が docs に無いことが判明した（石川の代表取締役在任期間、UNI が現在システムを使う権原、在任中に作られた Owner's Pass の帰属）。オーナーが同日に回答し、Owner's Pass のデプロイ日は Polygon のブロックで確認した。

### Decision
| # | 事実・決定 |
|---|---|
| F-1 | 石川の UNI 代表取締役の在任期間は **2021-01-15〜2025-09-15**。本件譲渡対象一式の企画着手は退任後の **2026-01-22**（04 §8。開発着手日として記録）であり、在任中の職務として制作されたものではない |
| F-2 | UNI は本システムを**利用許諾契約なく無償で使用**している。本件取得で権利と使用の不一致を解消する |
| F-3 | **Owner's Pass ERC721**（`0x1c19d0367236127a4d73c4816daee36fc23edd8a`、4,200 枚）は在任中の **2025-02-02**（Polygon ブロック 67,432,523）に **UNI の費用負担で外部委託により制作**された **UNI の資産**。機能が不足しているため、譲渡対象に含まれるオーナーチケット会員カード（OT カード）で置き換える想定。譲渡対象外 |
| F-4 | 譲渡資料・docs では「石川が開発した」と書かず、**「石川が企画した」に留める**。着手日は「企画に着手（開発着手日として記録）」と表記する。「売り手（開発者本人）」は「売り手（企画者本人）」、「個人で開発した」は「個人で企画した」に置き換える |
| F-5 | 石川が企画し、**一部の開発を外部業者に委託していたとしても、著作権は石川に帰属する**（オーナー確認 2026-09-24）。「IP はすべて石川」（03 §1）の前提は変わらない。DD で求められる帰属の証跡（03 §1）には、委託分の権利帰属を示す資料を含める |

### Reason
F-1〜F-3 は、100% 株主との取引で税務当局・評価人が最初に確認する IP 帰属の根拠になる。F-4 は制作主体の表現をオーナーの意向に合わせて統一するため。

### Consequences
- 反映済み: 13（§2.2・§4・§7・§9）、03 §1・§2.2、04 §8、05 §6、07 §1・§2、工数表 v1.2 ドラフト §1.1・§1.2・§4・§7・末尾、本ファイル U-12
- ヒアリング原本 `~/Downloads/ヒアリング項目_会計士税理士_UNI現代表_2026-09-15.txt` の前提行も「個人で企画した」に修正済み（回答部分は変更なし）
- 会計士・税理士ヒアリングで確認する項目に追加: 無償使用の課税上の扱い（制作主体と著作権帰属の整合は F-5 で決着、確認不要）
- 07 §2 の Owner's Pass 行にデプロイ日と帰属（F-3）を追記済み
