...
- 09 §2 の 3＋9 ヶ月の期間構成は維持するが、費用は U-3 のとおり無償（09 §5）。09 §2 が前提にする「UNI 側運用者」は置かず運用担当は石川 1 名のまま継続するため、集中引継ぎを「移管確認 3 ヶ月＋保守 9 ヶ月」に組み直した（同日反映、PR #65）
- 鍵管理 Part B: 署名者構成を「会社 MetaMask＋オーナー Ledger＋紙の冷蔵鍵」から U-9 に変更。members `ops/SEALED_BACKUP_RUNBOOK.md` の Safe 行（2-3・2-4、§3.3）、12 §1 の T+3・契約終了行、members `ops/KEY_MANAGEMENT_MIGRATION.md` Part B の手順は U-9・U-14 の構成で書き直し済み（同日反映、PR #64・#67）。UNI 側での Ledger の PIN・復旧フレーズの保管は SEALED_BACKUP §3.3 の手順（高橋代表が初期化・記入、オーナーは 24 語を見ない）で UNI 側が実施する。実施は Ledger 調達後
...
## Decision: 取得理由書のオーナー確認事項 8 件の回答と、関連 docs の不整合 4 点の修正（2026-09-24）

### Context
[13_UNI_ACQUISITION_RATIONALE.md](13_UNI_ACQUISITION_RATIONALE.md) 初稿の「オーナー確認事項」8 件にオーナーが回答した。併せて、2026-09-24 の U-1〜U-13 に対して古い記述が残っていた docs を同期した。

### Decision
| # | 事項 | 回答 |
|---|---|---|
| O-1 | 代金が役員貸付金の残高を下回る場合 | 残債は可能な限り速やかな解消を目指す |
| O-2 | 取得価額 | 評価書の時価と**同額**。評価額を**税込価格**として扱う |
| O-3 | 未リリース機能・OT カード | 取得対象に**含める**。評価書作成前に完成・リリースする方針。評価基準日までに完成していないコードは評価に含めず、譲渡価額は評価時点のコードに対する評価額で定める |
| O-4 | メタバース | **2027 年 2 月にリリース**し、既存 NFT 所有者へ提供するサービスとする |
| O-5 | 資産区分 | 対象一式をソフトウェアとして**一括計上**（5 年償却） |
| O-6 | 契約書の権利条項 | 著作権法 27 条・28 条の権利を含めて譲渡し、著作者人格権は不行使。条件は **UNI 側に有利**に定める |
| O-7 | U-5「顧問弁護士・税理士の事前確認はしない」 | 取得理由書に**記載しない** |
| O-8 | 決裁者所見 | 代表取締役が取得理由書の内容を **2026-09-24 に確認済み** |

### Consequences
- 13: §2.2・§3.3・§5・§6・§8・§9 B に反映。「オーナー確認事項」節は「未確定事項（外部の成果物・専門家回答待ち）」7 件に置き換え
- 不整合の修正: 07 §2・10 §1（会員データは譲渡対象外・通知不要、U-10）、08 §1.1・末尾（代表決裁・知財譲渡契約、U-5/U-6）、09 §1・§2（無償期間は譲渡翌月起点、U-3）、05 §2 A-1・§3.1（Part A 完了 9/21）・§3.5（S2 2 件）、03 §2.4・§5（代表決裁・初稿済み）
- O-2 の「評価額を税込価格として扱う」は、個人側の消費税の扱い（課税事業者か、内税か）と結び付くため、税理士ヒアリング（03 §2.4）で確認する
- O-3 により 09 §8「OT カードが譲渡日までに完了しない場合の別契約」の論点は、評価基準日を境に「完成分＝取得対象、未完成分＝評価外」と整理される
