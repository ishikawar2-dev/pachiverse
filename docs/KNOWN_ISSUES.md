...
- 他ページ（collection / faq / docs / litepaper / contracts / transparency）は 2026-09-21 に横展開済み: 閉じたモバイルメニューの `backdrop-filter` を開いている間だけに、走査線を `transform` ドリフトに、星 1 層の `background-position-x` アニメ（240 秒周期）は停止して静止画に（1.75 px/s は知覚できず、canvas 化するほどの負荷ではない）。`design-preview.html`（公開ナビから未リンク）も同日、星 3 層のドリフト停止と走査線の transform 化だけ適用（スクロール視差の `background-position-y` は残る。プレビュー用途なので許容）
