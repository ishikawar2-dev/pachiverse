...
| Cloudflare R2（オーナー個人 `ishikawar2@gmail.com` アカウント、バケット `pachiverse-media`） | 公式 PV 動画のホスティング（`/collection` のトレーラー枠。転送量無料）と、2026-09-15 からは Collection Explorer の機体画像 `assets/machines/{t,d}` 1,000 件（`api/collection.js` の既定 URL。`docs/R2_MACHINE_ASSETS_MIGRATION.md`）。公開 URL は `https://pub-4f767ce43f34417aa267bf5a563efdcf.r2.dev/`。独自ドメイン化には pachiverse.com の DNS を Cloudflare へ移す必要がある | なし（ダッシュボードで管理。元動画とWeb用書き出しは `pvm-art/pv/web/`） |
...
- [04_DEV_EFFORT_EVIDENCE.md](04_DEV_EFFORT_EVIDENCE.md) — 実装規模の証跡（Git 記録から `scripts/dev_effort_evidence.sh` で生成、工数表 v1.2 の付録。工数の証跡ではない）
