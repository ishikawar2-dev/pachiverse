...
└─────────────────────────────────────────────────────────────┘
               ↑ メタデータ・画像（IPFS）
┌─────────────────────────────────────────────────────────────┐
│ pvm-art（独立した入れ子 Git リポジトリ、2026-09-15〜）          │
...
| `pvm-art/` | **独立した入れ子 Git リポジトリ**（`pachiverse01-ai/pvm-art`、2026-09-15〜） | NFT アート生成パイプライン（Python）。原本画像 10GB・候補素材・PV 素材は Git 外（`out/MANIFEST_*.sha256` で照合） |
...
- `pvm-art` の原本画像（`out/images4096/`、10GB）は Git 外。**別置きバックアップが必要**（`pvm-art/README.md`「Git 管理と資産の所在」参照）。
