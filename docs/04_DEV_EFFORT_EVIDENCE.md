...
| 日付 | 事実 | 根拠（所在） |
|---|---|---|
| 2025 年 | 開発開始（GitHub 不使用、ローカルとサーバー直編集）| **要補強**: 契約・請求・設計資料など日付付きの一次資料をオーナーが提示（下記「収集すべきもの」） |
...
**収集すべきもの（オーナー）**: 2025 年分の日付付き資料。候補: レンタルサーバー・ドメインの契約/請求、デザイン・設計資料の作成日、外部 API（fal.ai / Pinata / GCP / Vercel）の請求履歴、ChatGPT・Codex・Claude の会話履歴のエクスポート（開始日）、UNI スタッフとのやり取り（サポート運用開始日）、Time Machine 等のバックアップに残る旧作業ディレクトリ（`~/Documents/Point-Pocket/poiverse-member` 等、現在は移動済みで不在）。
